...
def call_llm(system_prompt, dynamic_context):
    ...
    payload = {
        ...
        "stream": False
        # "format": "json"
        # Do NOT use "format": "json" here. Reasoning/DeepSeek models will often return an EMPTY string 
        # because the internal <think> tag violates the strict JSON schema check of the Ollama engine.
    }
    
    logging.info(f"Sending request to Ollama ({model})...")
    
    try:
        # Increased timeout to 1200 seconds (20 minutes) to accommodate heavy reasoning models
        ...
        # Optional: aggressively strip trailing/leading double braces if LLM output `{{` instead of `{`
        ...
        try:
            ...
            # Fallback 1: Try fixing literal newlines/tabs inside string values
            in_string = False
            escaped = False
            fixed_chars = []
            ...
            try:
                ...
                # Fallback 2: Check if the model used single quotes instead of double quotes for keys/values
                ...
                # Additional fallback: return the exact error block to the context loop
